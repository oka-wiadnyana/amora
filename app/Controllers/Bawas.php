...
class Bawas extends BaseController
{
    private $validation;
    ...
    public function data_laporan()
    {
        ...
        return view('bawas/laporan_bawas');
    }

    public function data_bawas_datatable($jenis_laporan = null)
    {


        $queryBuilder = db_connect()->table('bawas a')->where('jenis_laporan', $jenis_laporan);
        ...
        $datatables->format('file', function ($value, $row) {
            $btn = '<a href="' . base_url('file_bawas/' . $value) . '" target="_blank"><span class="mdi mdi-file-document text-danger h2"></span></a>';
            return $btn;
        });
    ...
    public function modal_laporan()
    {
        ...
        $data_laporan = db_connect()->table('bawas')->where('id', $id)->get()->getRowArray();
        ...
        return $this->response->setJSON([view('bawas/modal_laporan', ['tahuns' => $tahun, 'data_laporan' => $data_laporan])]);
    }
    ...
    public function insert_laporan($ubah = null)
    {
        ...
        if (!$ubah) {
            if (!$this->validate([
                ...
                return redirect()->to('bawas/data_laporan');
            }
        }
        ...
        $is_data_exist = db_connect()->table('bawas')->where('tahun', $tahun)->where('jenis_laporan', $jenis_laporan)->get()->getRowArray();
        if (!$ubah) {
        ...
        if (!$ubah) {
            if (db_connect()->table('bawas')->insert($data_insert)) {
        ...
        if ($ubah) {
            $id = $this->request->getVar('id');
            if (db_connect()->table('bawas')->where('id', $id)->update($data_insert)) {
    ...
    {
        $id = $this->request->getVar('id');
        if (db_connect()->table('bawas')->where('id', $id)->delete()) {
