...
class Pengawasanlainnya extends BaseController
{
    private $validation;
    ...
    public function data_laporan()
    {
        ...
        return view('pengawasanlainnya/laporan_pengawasanlainnya');
    }

    public function data_pengawasanlainnya_datatable($jenis_laporan = null)
    {


        $queryBuilder = db_connect()->table('pengawasanlainnya a')->where('jenis_laporan', $jenis_laporan);
        ...
        $datatables->format('file', function ($value, $row) {
            $btn = '<a href="' . base_url('file_pengawasanlainnya/' . $value) . '" target="_blank"><span class="mdi mdi-file-document text-danger h2"></span></a>';
            return $btn;
        });
        ...
        $datatables->only(['instansi', 'tahun', 'tanggal_laporan', 'file']);
    ...
    public function modal_laporan()
    {
        ...
        $data_laporan = db_connect()->table('pengawasanlainnya')->where('id', $id)->get()->getRowArray();
        ...
        return $this->response->setJSON([view('pengawasanlainnya/modal_laporan', ['tahuns' => $tahun, 'data_laporan' => $data_laporan])]);
    }
    ...
    public function insert_laporan($ubah = null)
    {
        ...
        if (!$ubah) {
            if (!$this->validate([
                ...
                return redirect()->to('pengawasanlainnya/data_laporan');
            }
        }
        ...
        $is_data_exist = db_connect()->table('pengawasanlainnya')->where('tahun', $tahun)->where('jenis_laporan', $jenis_laporan)->get()->getRowArray();
        if (!$ubah) {
        ...
        if ($file_doc->isValid()) {
            ...
            $file_doc->move('file_pengawasanlainnya', $file);
        } else {
            $file = $this->request->getVar('file_lama');
        }

        $data_insert = compact('instansi', 'tahun', 'tanggal_laporan', 'file', 'jenis_laporan');
        if (!$ubah) {
            if (db_connect()->table('pengawasanlainnya')->insert($data_insert)) {
        ...
        if ($ubah) {
            $id = $this->request->getVar('id');
            if (db_connect()->table('pengawasanlainnya')->where('id', $id)->update($data_insert)) {
    ...
    {
        $id = $this->request->getVar('id');
        if (db_connect()->table('pengawasanlainnya')->where('id', $id)->delete()) {
