...
class Eis extends BaseController
{
    ...
    public function daftar_eis_ajax()
    {
        ...
        $penetapan_pp = db_connect('sipp')->table('perkara a')->select('a.perkara_id, nomor_perkara, DATE_FORMAT(tanggal_pendaftaran, "%d-%m-%Y") as tanggal_pendaftaran')->selectSubquery(db_connect('sipp')->table('perkara_panitera_pn b')->selectMin('tanggal_penetapan')->where('b.perkara_id=a.perkara_id'), 'tanggal_penetapan_pp')->having('tanggal_penetapan_pp !=', null)->where('MONTH(tanggal_pendaftaran)', $bulan)->where('YEAR(tanggal_pendaftaran)', $tahun)->get()->getResultArray();
        // dd($penetapan_pp);
        $data_pen_pp = [];
 ...
 (SELECT tanggal_sidang FROM perkara_jadwal_sidang s1
    ...
    public function get_skor()
    {
        ...
        $now = date('Y-m-d');
        $data_eis_db = db_connect()->table('nilai_eis')->orderBy('date', 'desc')->limit(1)->get()->getRowArray();
        if (!$data_eis_db) {

            $data_eis = $client->get('http://localhost:4000/getdataeis');
            $data = json_decode($data_eis->getBody(), true);

            db_connect()->table('nilai_eis')->insert(['date' => $now, 'past_rank' => $data['response']['past']['peringkat'], 'past_score' => $data['response']['past']['skor'], 'current_rank' => $data['response']['now']['peringkat'], 'current_score' => $data['response']['now']['skor']]);

            $data_eis = db_connect()->table('nilai_eis')->where('date', $now)->get()->getRowArray();
            $data_response = [
                'date' => $data_eis['date'],
                'past_rank' => $data_eis['past_rank'],
                'past_score' => $data_eis['past_score'],
                'current_rank' => $data_eis['current_rank'],
                'current_score' => $data_eis['current_score'],
            ];
            return $this->response->setJSON($data_response);
        }

        if ($data_eis_db['date'] == $now) {
            $data_eis = db_connect()->table('nilai_eis')->where('date', $now)->get()->getRowArray();
            $data_response = [
                'date' => $data_eis['date'],
                'past_rank' => $data_eis['past_rank'],
        ...
        if ($data_eis_db['date'] < $now && time() <= strtotime('08:30:00') || ($data_eis_db['date'] == $now && time() < strtotime('08:30:00'))) {
            $data_eis = db_connect()->table('nilai_eis')->orderBy('date', 'desc')->limit(1)->get()->getRowArray();
            $data_response = [
                'date' => $data_eis['date'],
                'past_rank' => $data_eis['past_rank'],
                'past_score' => $data_eis['past_score'],
