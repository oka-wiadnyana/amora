...
class Briefing extends BaseController
{
    ...
    public function data_laporan($level = null)
    {
        ...
        return view('briefing/laporan_briefing', ['level' => $level]);
    }

    public function data_briefing_datatable($level = null, $jenis_laporan = null)
    {


        $queryBuilder = db_connect()->table('briefing_ptsp a')->select('a.id as id_briefing,a.*')->where('level', $level)->where('jenis_laporan', $jenis_laporan)->orderBy('tanggal', 'desc');


        $datatables = new DataTables($queryBuilder, '4');
        $datatables->addColumn('action', function ($row) {

            $btn = '<a class="btn btn-warning edit-btn" data-id="' . $row->id_briefing . '">Edit</a> <a class="btn btn-danger hapus-btn" data-id="' . $row->id_briefing . '">Hapus</a>';
            return $btn;
        });

        $datatables->format('file', function ($value, $row) {
            $btn = '<a href="' . base_url('file_briefing/' . $value) . '" target="_blank"><span class="mdi mdi-file-document text-danger h2"></span></a>';
            return $btn;
        });
    ...
    public function modal_laporan($level = null)
    {
        ...
        $data_laporan = db_connect()->table('briefing_ptsp')->where('id', $id)->get()->getRowArray();


        return $this->response->setJSON([view('briefing/modal_laporan', ['level' => $level, 'data_laporan' => $data_laporan])]);
    }
    ...
    public function insert_laporan($ubah = null)
    {
        ...
        if (!$ubah) {
            if (!$this->validate([
                ...
                return redirect()->to('briefing/data_laporan/' . $level);
            }
        }
        ...
        $is_data_exist = db_connect()->table('briefing_ptsp')->where('tanggal', $tanggal)->where('jenis_laporan', $jenis_laporan)->where('level', $level)->get()->getRowArray();
        if (!$ubah) {
        ...
        if ($file_doc->isValid()) {
            ...
            $file_doc->move('file_briefing', $file);
        } else {
        ...
        if (!$ubah) {
            if (db_connect()->table('briefing_ptsp')->insert($data_insert)) {
        ...
        if ($ubah) {
            $id = $this->request->getVar('id');
            if (db_connect()->table('briefing_ptsp')->where('id', $id)->update($data_insert)) {
    ...
    {
        $id = $this->request->getVar('id');
        if (db_connect()->table('briefing_ptsp')->where('id', $id)->delete()) {
