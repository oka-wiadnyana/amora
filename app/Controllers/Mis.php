...
class Mis extends BaseController
{
  ...
  public function mis_ajax()
  {
    ...
    $data_15 = db_connect('sipp')->query('SELECT nomor_perkara, DATE_FORMAT(tanggal_putusan, "%d-%m-%Y") as tanggal_putusan FROM perkara_putusan LEFT JOIN perkara ON perkara_putusan.perkara_id = perkara.perkara_id WHERE YEAR(tanggal_putusan)=YEAR(CURDATE()) AND tanggal_putusan IS NOT NULL AND (jenis_perkara_nama = "Perceraian" OR jenis_perkara_nama = "Permohonan Pengangkatan Anak" OR jenis_perkara_nama = "Wasiat" OR jenis_perkara_nama = "Kejahatan Terhadap Kesusilaan" OR jenis_perkara_nama = "Kekerasan Dalam Rumah Tangga" OR jenis_perkara_nama = "Perlindungan Anak" OR alur_perkara_id =118) AND amar_putusan_anonimisasi_dok IS NULL')->getResultArray();
    ...
    // return $this->response->setJSON($mis_show);
