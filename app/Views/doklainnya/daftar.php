  ...
  <div class="row mb-2">
    <div class="col">
      <span class="h3">
        ...
        Dokumen Lainnya <?= $unit->nama_bagian; ?>
      </span>
    </div>
  </div>
  <div class="row">
  ...
  $(document).ready(function() {
    ...
    function table_datatable() {
      table = $('#table-data').DataTable({
        ...
        "order": [],
        "ajax": {
          "url": `<?= base_url('doklainnya/data_datatable/' . $unit->level); ?>`,
          "type": "POST"
        },
        "columnDefs": [{
            "target": 0,
    ...
    $('.tambah-btn').click(function(e) {
      ...
      $.ajax({
        type: "get",
        url: "<?= base_url('doklainnya/modal_tambah/' . $unit->level); ?>",
        ...
        if (result.isConfirmed) {
          $.ajax({
            ...
            success: function(response) {
              ...
              $(location).attr('href', `<?= base_url('doklainnya/daftar/' . $unit->level); ?>`);
            }
          });
        }
      })
